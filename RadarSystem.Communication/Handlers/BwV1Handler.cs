...
namespace RadarSystem.Communication.Handlers
{
    ...
    /// 端口: 11110
    /// 协议: 自定义文本协议
    /// 注册消息格式: "registMsg" + SlaveIdHex
    /// </summary>
    public class BwV1Handler : SimpleChannelInboundHandler<byte[]>
    {
        private readonly ILogger<BwV1Handler> _logger;
        ...
        public override void ChannelActive(IChannelHandlerContext context)
        {
            ...
            _logger.LogInformation($"[北纬 V1] 连接建立，远程地址: {remoteAddress}");
        ...
        {
            var remoteAddress = context.Channel.RemoteAddress;
            _logger.LogInformation($"[北纬 V1] 连接断开，远程地址: {remoteAddress}");
        ...
        protected override void ChannelRead0(IChannelHandlerContext ctx, byte[] msg)
        {
            ...
            {
                _logger.LogError(ex, "[北纬 V1] 处理数据时发生异常");
            }
        }

        public override void ExceptionCaught(IChannelHandlerContext context, Exception exception)
        {
            _logger.LogError(exception, "[北纬 V1] 通道异常");
            context.CloseAsync();
        }
        ...
        private void HandleData(byte[] msgBytes, IChannelHandlerContext ctx)
        {
            try
            {
                ...
                _logger.LogDebug($"[北纬 V1] 接收数据: {msgStr}");
            ...
            {
                _logger.LogError(ex, "[北纬 V1] 处理数据失败");
            }
        }
        ...
        private void HandleRegistration(string msgStr, IChannelHandlerContext ctx)
        {
            try
            {
                _logger.LogInformation("[北纬 V1] 收到设备注册消息");
                ...
                if (!string.IsNullOrEmpty(slaveIdHex))
                {
                    ...
                    _logger.LogInformation($"[北纬 V1] 设备注册成功，SlaveId: {deviceId} (Hex: {slaveIdHex})");
                ...
                {
                    _logger.LogWarning("[北纬 V1] 注册消息中未找到 SlaveId");
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[北纬 V1] 处理注册消息失败");
            }
        }
        ...
        private void SendRegistrationResponse(IChannelHandlerContext ctx, string deviceId)
        {
            try
            {
                ...
                _logger.LogDebug($"[北纬 V1] 发送注册响应，设备ID: {deviceId}");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[北纬 V1] 发送注册响应失败");
            }
        }
        ...
        private void HandlePositionData(byte[] msgBytes, string msgStr, IChannelHandlerContext ctx)
        {
            try
            {
                ...
                _logger.LogInformation($"[北纬 V1] 收到定位数据，长度: {msgBytes.Length}");
                ...
                _logger.LogDebug($"[北纬 V1] 定位数据已加入处理队列");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[北纬 V1] 处理定位数据失败");
            }
        }
        ...
        {
            _logger.LogInformation("[北纬 V1] 数据处理线程已启动");
            ...
            foreach (var bwData in _dataQueue.GetConsumingEnumerable())
            {
                ...
                {
                    _logger.LogError(ex, "[北纬 V1] 处理队列数据失败");
                }
            }
        }
        ...
        private void SaveBwData(BwV1Data bwData)
        {
            try
            {
                ...
                _logger.LogInformation($"[北纬 V1] 数据已保存到文件: {filePath}");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[北纬 V1] 保存数据失败");
            }
        }
        ...
        private void PublishToMqtt(BwV1Data bwData)
        {
            try
            {
                string topic = $"/bw/v1/{bwData.DeviceId}/data";
                ...
                _logger.LogDebug($"[北纬 V1] MQTT消息准备发送 - Topic: {topic}");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[北纬 V1] 发送MQTT消息失败");
            }
        }
