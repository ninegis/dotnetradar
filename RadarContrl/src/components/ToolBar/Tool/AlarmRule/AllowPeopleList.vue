  ...
  <section id="idalarmcontactlist" v-show="visible" class="">
    <DragContainer :dragger-width="store.dragContainer.width">
      <template v-slot:dragger-header>
        ...
        <span class="dragger-header">&nbsp;&nbsp;&nbsp;{{$t('backend.allowPeople')}}</span>
      </template>
      <template v-slot:dragger-content>
        <a-row type="flex" :gutter="16" align="middle" class="custome-row">
          <a-button class="custom-ant-btn" type="primary" ghost block @click="store.alarmContactInfo=null;store.toolbarcontent='allowPeopleInfo'">{{$t('backend.addAutorizedPerson')}}</a-button>
        </a-row>
        <a-row>
          <el-table :data="contactData" style="width: 100%">
            <el-table-column prop="name" :label="$t('common.fullName')"/>
...
const itemDelete = (row)=>{
  ApiRadar.deleteAllowPeople(row.id).then(res=>{
    showMessage(res.data.data);
    dataInit();
  })
}
const dataInit=()=>{
  ApiRadar.getAllowPeople(store.sysinfo.config.projectCode).then(res=>{
